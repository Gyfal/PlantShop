...
local function getStatus(data, idSemya)
    ...
    if not tStatus[status] then return end
...
addEventHandler( "onClientKey", getRootElement(), function(button,press) 
    if button == "escape" and press and tWeb["load"] then
        triggerEvent('plantShop:closedGUI', resourceRoot)
        cancelEvent()
    end 
    if button == "mouse_wheel_up" or button == "mouse_wheel_down" then
        ...
        if PlayerData['semena'][selector] and PlayerData['semena'][selector] == true then
            PlayerData['select'] = selector 
            return
        end 
        if PlayerData['semena'][selector + 1] and PlayerData['semena'][selector + 1] == true then
            PlayerData['select'] = selector + 1
            return
        end 
        return true
    end
...
local function dxDrawLoading (x, y, width, height, x2, y2, size, color, color2, second)
